 ...
 export default function FormPage() {
  ...
  const fetchFormDefinition = async (formName, version = '') => {
   ...
   try {
    ...
    if (!res.ok) {
     console.error(`Error fetching form definition: ${res.status} ${res.statusText} for URL: ${url}`);
    ...
    if (data) {
     ...
     if (data.versions && Array.isArray(data.versions)) {
      ...
      // Always set selectedVersion to the *first* version on initial load
  ...
  // useEffect to fetch form definition when selectedForm changes - MODIFIED
  useEffect(() => {
   if (selectedForm) {
    // Fetch with selectedVersion, which will be '' initially, then updated.
    fetchFormDefinition(selectedForm, selectedVersion);
   }
  }, [selectedForm]); // Removed selectedVersion from dependency array


  // useEffect to load selectedVersion from localStorage on initial load - REMOVED
  // useEffect(() => {
  //  if (selectedForm) {
  ...
  const handleFormSend = async (action) => {
   ...
   try {
    ...
    const isRenamingCurrentVersion = loadedFormDefinition && selectedVersion !== loadedFormDefinition.version_name && !isCloning;
    ...
    else { // SAVE/SUBMIT (no clone, no rename): simply save changes to the *current* version
     payload.append('version_name', selectedVersion); // Current version name (for save/submit to existing version)
    }
    ...
    localStorage.setItem(
     LOCAL_STORAGE_SUBMISSION_KEY(selectedForm, data.version_name), // Use data.version_name here!
    ...
    setSelectedVersion(data.version_name); // Update selectedVersion to the *new* version name from response
    localStorage.setItem(LOCAL_STORAGE_VERSION_KEY(selectedForm), data.version_name); // Store the new version in local storage

    // **CHANGE:** Explicitly re-fetch form definition with the *new* version from backend response
    if (selectedForm && data.version_name) { // Ensure selectedForm and version are available
     await fetchFormDefinition(selectedForm, data.version_name); // Re-fetch with the *new* version
    }
          ...
          <Input
           ...
           onChange={(e) => {
            ...
            localStorage.setItem(LOCAL_STORAGE_VERSION_KEY(selectedForm), newVersion); // Store version on input change
             setIsCloning(false); // **Ensure isCloning is set to false when typing directly**
             setClonedFromVersion(''); // Clear clonedFromVersion as it's not a clone operation
